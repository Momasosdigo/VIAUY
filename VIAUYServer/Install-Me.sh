...
# Verificar si el directorio no existe, de ser asi se prosigue
    ...
    if [[ $installChoice =~ [Yy] ]]; then
        ...
        if mkdir -p "$HOME"/"$dir"; then 
    #      \_____________/
    #             / 
    # Comprueba si se creo la carpeta y si se creó correctamente se continua.
            sudo rsync -rav "$HOME/VIAUYServer/Container/" "$HOME/$dir/Container/"
        #        \________/
        #           /
    ...
    echo -e "\n[ El directorio [ ""$HOME""/""$dir"" ] existe y se moverán los datos a continuación... ]"
    sleep 1.5s

    if sudo rsync -r "$HOME/VIAUYServer/Container/" "$HOME/$dir/Container/"; then
